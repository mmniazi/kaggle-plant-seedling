...
def main():
    ...
    subdir = os.path.join(args.data, 'submissions')
    ...
    test_loader = torch.utils.data.DataLoader(
        TestImageFolder(args.data, testdir, transforms.Compose([
...
def test(test_loader, model, subdir):
    ...
    for i, (images, filepath) in enumerate(test_loader):
        ...
        max_index = np.argmax(smax_out.data.cpu().numpy())
        csv_map[filepath + '.png'] = class_labels[max_index]

    sub_path = subdir + '/{0}epoch'.format(epochs) + '.csv'
